...
uv add polars[all] plotly[express] statsmodels palmerpenguins nycflights13
```
...
::: {.callout-tip}
This method uses something called List Comprehension, which you can learn more about [here](https://www.w3schools.com/python/python_lists_comprehension.asp). We won't need these techniques for this class, but you should be aware of them.
:::
...
Note that the variable type information you set up is lost when you save to CSV because you're starting over with reading from a plain text file. 
This makes CSVs somewhat unreliable for caching interim results. You need to recreate the column specification every time you load in. 

There are two better alternative is [Parquet files](https://www.databricks.com/glossary/what-is-parquet). These maintain the column types and are generally faster and more space-efficient.
...
Parquet is becoming more common in use with data sciences, but they have not caught on with more business based analytical tools like Excel, Tableau, & Power BI, so CSVs still reign supreme.
